...
const useUserResolvedChainedSpeedMarketsDataQuery = (
    ...
    return useQuery<ChainedSpeedMarket[]>({
        queryKey: QUERY_KEYS.User.ResolvedChainedSpeedMarkets(queryConfig, walletAddress),
        queryFn: async () => {
            ...
            const chainedSpeedMarketAMMContract = getContract({
                abi: chainedSpeedMarketsAMMContract.abi,
                address: chainedSpeedMarketsAMMContract.addresses[queryConfig.networkId] as any,
                client: queryConfig.client,
            }) as ViemContract;

            const ammParams = await speedMarketDataContract.read.getChainedSpeedMarketsAMMParameters([walletAddress]);
            ...
            const maturedMarkets = await chainedSpeedMarketAMMContract.read.maturedMarketsPerUser([
                index,
                pageSize,
                walletAddress,
            ]);
