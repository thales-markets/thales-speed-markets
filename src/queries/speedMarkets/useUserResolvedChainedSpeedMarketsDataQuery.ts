...
const useUserResolvedChainedSpeedMarketsDataQuery = (
    ...
    return useQuery<ChainedSpeedMarket[]>({
        queryKey: QUERY_KEYS.User.ResolvedChainedSpeedMarkets(networkId, walletAddress),
        queryFn: async () => {
            ...
            if (chainedSpeedMarketsAMMContract && speedMarketsDataContract) {
                ...
                const maturedMarkets = await chainedSpeedMarketsAMMContract.read.maturedMarketsPerUser([
                    index,
                    pageSize,
                    walletAddress,
                ]);
