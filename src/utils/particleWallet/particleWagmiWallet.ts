...
export function particleWagmiWallet(param?: any) {
    type Provider = EIP1193Provider;
    type Properties = any;

    return createConnector<Provider, Properties>((config: any) => ({
        ...
        async connect({ chainId }: { chainId: number }) {
            try {
                ...
                const accounts = (await (provider as any).connect(param)).map((x: any) => getAddress(x));
